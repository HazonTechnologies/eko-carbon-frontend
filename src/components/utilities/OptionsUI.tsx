...
const OptionsUI: NextPage<OptionsPropType> = ({ options, select, selected }) => (
    ...
    {options.length &&
      options.map((option) => (
        <button
          ...
          className={`flex items-center shadow-1 ${selected && selected.value === option.value ? 'border border-tertiary-mid' : ''} bg-secondary-high px-5 rounded-md py-3 w-[350px] gap-x-4 hover:border hover:border-tertiary-mid`}
          key={option.title}
        >
