...
export default function CustomButton({ width,htmlType, icon, color, bg, disabled,onClickTrigger, children }: Props) {
  return (
    ...
    disabled={disabled}
      ...
      `}
    >
    {icon && <Image unoptimized={true} loader={imageLoader} src={`/assets/icons/${icon}`} height={15} width={15} alt={`${icon?.split('.')[0]?icon?.split('.')[0]:icon}`} />} 
     <span>
    {children}
     </span>
