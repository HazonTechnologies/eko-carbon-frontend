...
const PreAssessment = () => {
  ...
  const [isCreditCardModal, setCreditCardModal] = useState<boolean>(false);
  const onSubmit = (values: any) => {
    setCreditCardModal(true);
    console.warn(values);
    // fetcher("PreAssessmentCalculator/Initiate-Transaction").then((res) => {
    //   console.log(res);
    //   postApi("PreAssessmentCalculator/Calculate-Recyclable-Material", {
    //     ...values,
  ...
  return (
    <>
      <CreditCard
        isModalVisible={isCreditCardModal}
        setIsModalVisible={setCreditCardModal}
      />
